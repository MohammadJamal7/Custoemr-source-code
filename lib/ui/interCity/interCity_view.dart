...
class InterCityView extends StatefulWidget {
  InterCityView({super.key});

  @override
  State<InterCityView> createState() => _InterCityViewState();
}

class _InterCityViewState extends State<InterCityView> {
  SuperTooltip? tooltip;
  late InterCityController controller;

  @override
  void initState() {
    super.initState();
    // Get or create the controller
    controller = Get.put(InterCityController());
    // Refresh services every time the page loads
    WidgetsBinding.instance.addPostFrameCallback((_) {
      controller.refreshServices();
    });
  }

  final Map<String, String> intercityTooltips = {
    'مشتركة مع ركاب':
    ...
    'رحلة خاصة بين المدن':
        'إذاكنت بحاجة إلى باص للسفر بين المدن أو للخطوط الطويلة نوع تويوتا أو أوربي وغيره',
    'رحلة مجدولة':
        'للحجز المسبق ،إذا كنت تخطط للسفر أو مشوار في موعد وتاريخ معين داخل المدينة والمدن الأخرى',
    'رحلة بين المدن وتوصيل':
        'إذاكنت بحاجة إلى باص للسفر بين المدن أو للخطوط الطويلة نوع تويوتا أو أوربي وغيرة',
    'رحلة بين المدن ':
        'إذاكنت بحاجة إلى باص للسفر بين المدن أو للخطوط الطويلة نوع تويوتا أو أوربي وغيره',
    ' رحلة بين المدن':
        'إذاكنت بحاجة إلى باص للسفر بين المدن أو للخطوط الطويلة نوع تويوتا أو أوربي وغيره',
    'حجز رحلة بين المدن وتوصيل':
        'إذاكنت بحاجة إلى باص للسفر بين المدن أو للخطوط الطويلة نوع تويوتا أو أوربي وغيره',
  };
                                ...
                                children: [
                                        ...
                                        if (Constant.selectedMapType == 'osm') {
                                          ...
                                          Get.to(const GoogleMapSearchPlacesApi())!
                                            ...
                                            if (value != null) {
                                              ...
                                              // Use Arabic formatted address for city and location
                                              controller.sourceCityController
                                                      .value.text =
                                                  placeDetailsModel
                                  ...
                                  SizedBox(
                                    height: Responsive.height(22,
                                        context), // Increased to accommodate card height and padding
                                    child: ListView.builder(
                                      ...
                                      itemBuilder: (context, index) {
                                        ...
                                        print(
                                            'NAME SERVICE -> ${Constant.localizationName(serviceModel.name).trim()}');
                                        print(
                                            'Contains Key: ${intercityTooltips.containsKey(Constant.localizationName(serviceModel.name).trim())}');
                                        print(
                                            'Is Selected: ${controller.selectedInterCityType.value.id == serviceModel.id}');
                                        return Obx(
                                          () => InkWell(
                                            onTap: () {
                                              ...
                                              child: Container(
                                                ...
                                                child: Stack(
                                                  children: [
                                                    ...
                                                    if (intercityTooltips.containsKey(
                                                  ...
                                                  child: Padding(
                                                      ...
                                                      child: Center(
                                                        child: RichText(
                                                          text: TextSpan(
                                                            text:
                                                                '${'Approx time'.tr} ${Constant.getLocalizedTime(controller.duration.value)}. ${"Approx distance".tr} ${controller.distance.value.toString()} ${Constant.getLocalizedDistanceUnit()}',
                                                            style: GoogleFonts
                                                                .poppins(
                                                                    color: Colors
                                                                        .black),
  ...
  _buildSubmitButton(BuildContext context, InterCityController controller,
    ...
    return ButtonThem.buildButton(
      ...
      btnWidthRatio: 0.9,
      onPress: () async {
        // Check if user is guest before placing the ride
        ...
        // double suggestPrice = Constant.getAmountShow(amount: controller.amount.value);
        ...
        if (controller.userModel.value.isActive == false) {
          ...
          bool isPaymentNotCompleted =
              await FireStoreUtils.paymentStatusCheckIntercity();
          bool isCurrentInterCityRideCheck =
              await FireStoreUtils.currentCheckInterCityRideCheck();

          if (isPaymentNotCompleted) {
            showAlertDialog(context);
            // showDialog(context: context, builder: (BuildContext context) => warningDailog());
          } else if (isCurrentInterCityRideCheck) {
            ...
            if (foundZones.isNotEmpty) {
              // Set selectedZone to the first found zone instead of always using zoneList[0]
              controller.selectedZone.value = foundZones[0];
              if (controller.selectedInterCityType.value.id ==
                  "647f350983ba2") {
                if (controller.sourceLocationController.value.text.isEmpty) {
                ...
                } else if (controller.selectedInterCityType.value.offerRate ==
                        true &&
                    controller.offerYourRateController.value.text.isEmpty) {
                  ...
                  for (var element in controller.images) {
                    Url url = await Constant()
                        .uploadChatImageToFireStorage(File(element.path));
                  ...
                  intercityOrderModel.sourceLocationName =
                      controller.sourceLocationController.value.text;

                  // Fix for empty sourceCity when creating rides directly from intercity page
                  if (controller.sourceCityController.value.text.isEmpty &&
                      controller
                          .sourceLocationController.value.text.isNotEmpty) {
                    // If sourceCity is empty but we have a location name, use that as fallback
                    intercityOrderModel.sourceCity =
                        controller.sourceLocationController.value.text;
                    log("⚠️ sourceCity was empty, using sourceLocationName as fallback");
                  } else {
                    intercityOrderModel.sourceCity =
                        controller.sourceCityController.value.text;
                  }

                  intercityOrderModel.sourceLocationLAtLng =
                      controller.sourceLocationLAtLng.value;

                  intercityOrderModel.parcelImage = parcelImages;
                  intercityOrderModel.parcelWeight =
                      controller.parcelWeight.value.text;
                  intercityOrderModel.parcelDimension =
                      controller.parcelDimension.value.text;

                  intercityOrderModel.destinationLocationName =
                      controller.destinationLocationController.value.text;
                  intercityOrderModel.destinationCity =
                      controller.destinationCityController.value.text;
                  intercityOrderModel.destinationLocationLAtLng =
                  ...
                  intercityOrderModel.offerRate =
                      controller.selectedInterCityType.value.offerRate == true
                          ? controller.offerYourRateController.value.text
                          : controller.amount.value;
                  intercityOrderModel.intercityServiceId =
                      controller.selectedInterCityType.value.id;
                  intercityOrderModel.intercityService =
                      controller.selectedInterCityType.value;
                  ...
                  intercityOrderModel.position = Positions(
                  ...
                  intercityOrderModel.whenTime =
                      DateFormat("HH:mm").format(controller.dateAndTime!);
                  intercityOrderModel.whenDates =
                      DateFormat("dd-MMM-yyyy").format(controller.dateAndTime!);
                  ...
                  intercityOrderModel.zoneIds =
                      foundZones.map((e) => e.id).toList();
                  intercityOrderModel.zones = foundZones;
                  intercityOrderModel.duration = controller.duration.value;
                  intercityOrderModel.adminCommission = controller
                              .selectedInterCityType
                              .value
                              .adminCommission!
                              .isEnabled ==
                  ...
                  // send alert
                  log("🔍 Sending intercity order notifications with originalServiceId: ${controller.selectedInterCityType.value.originalServiceId}");
                  FireStoreUtils()
                      .sendIntercityOrderData(
                          intercityOrderModel,
                          controller
                              .selectedInterCityType.value.originalServiceId!)
                      .listen((event) {
                    event.forEach((element) async {
                      print("FCM TOKEN LIST ${element.fcmToken}");
                      if (element.fcmToken != null) {
                        print("FCM TOKEN LIST ${element.fcmToken}");
                        Map<String, dynamic> playLoad = <String, dynamic>{
                          "type": "intercity_order",
                          "orderId": intercityOrderModel.id
                        };
                  ...
                  await FireStoreUtils.setInterCityOrder(intercityOrderModel)
                      .then((value) {
                    ShowToastDialog.closeLoader();
                    if (value == true) {
                      ShowToastDialog.showToast("Ride Placed successfully".tr);
                      // ✅ CRITICAL FIX: Navigate to intercity orders screen to show active ride
                      Get.offAll(() => const DashBoardScreen());
                      // Set the drawer index to intercity orders (index 3)
                      controller.dashboardController.selectedDrawerIndex(3);
                    }
                  });
                }
              } else if (controller.selectedInterCityType.value.id ==
                  "Kn2VEnPI3ikF58uK8YqY") {
                if (controller.sourceLocationController.value.text.isEmpty) {
                  ShowToastDialog.showToast("Please select source location".tr);
                } else if (controller
                  ...
                  ShowToastDialog.showToast("Please select date and time".tr);
                } else if (controller.selectedFreightVehicle.value.id == null ||
                    controller.selectedFreightVehicle.value.id!.isEmpty) {
                  ShowToastDialog.showToast(
                      "Please select cargo vehicle size.".tr);
                } else {
                  ShowToastDialog.showLoader("Please wait".tr);
                ...
                if (controller.sourceLocationController.value.text.isEmpty) {
                  ...
                  intercityOrderModel.whenDates =
                      DateFormat("dd-MMM-yyyy").format(controller.dateAndTime!);
                  intercityOrderModel.numberOfPassenger =
                      controller.noOfPassengers.value.text;
                  intercityOrderModel.comments =
                      controller.commentsController.value.text;
                  intercityOrderModel.otp = Constant.getReferralCode();
                  intercityOrderModel.taxList = Constant.taxList;
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            Expanded(
                              child: Text(
                                "Which vehicle is suitable for your cargo?".tr,
                                style: GoogleFonts.poppins(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        InkWell(
                          onTap: () async {
                            try {
                              ...
                              final exists = controller.contactList.any((c) =>
                                  (c.fullName ?? "") ==
                                  (contactModel.fullName ?? ""));
                              if (!exists) {
                              ...
                              // picker cancelled or failed; optionally show a toast
                            }
                          },
                          child: Padding(
                            padding: const EdgeInsets.symmetric(
                        ...
                        ButtonThem.buildButton(
                          context,
                          title:
                              "${"احجز لـ".tr} ${controller.selectedTakingRide.value.fullName?.tr}",
                          btnWidthRatio: 1,
  ...
  onCameraClick(BuildContext context, InterCityController controller) {
