...
class GoogleMapSearchPlacesApiState extends State<GoogleMapSearchPlacesApi> {
  ...
  void getSuggestion(String input) async {
    try {
      ...
      String request = '$baseURL?input=$input&key=${Constant.mapAPIKey}&sessiontoken=$_sessionToken&language=ar';
      var response = await http.get(Uri.parse(request));
      if (response.statusCode == 200) {
        setState(() {
          print('Google Places Autocomplete Response: ${response.body}');
  ...
  Future<PlaceDetailsModel?> getLatLang(String placeId) async {
    ...
    try {
      ...
      String request = '$baseURL?placeid=$placeId&key=${Constant.mapAPIKey}&language=ar';
      var response = await http.get(Uri.parse(request));
      // if (kDebugMode) {
      //   log(response.body);
      // }
      if (response.statusCode == 200) {
        print('Google Places Details Response: ${response.body}');
